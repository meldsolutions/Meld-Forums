...
<cfcomponent name="MeldForumsRemoteAOP" 
			displayname="MeldForumsRemoteAOP:RemoteProxyBean" 
			hint="Abstract Base Class for Aop Based Remote Proxy Beans" 
			output="false">
			
	<cfset variables.proxyId = CreateUUId() />
	<cfset variables.beanFactoryName = "MeldForumsBeanFactory" />
		...
		<cflock name="RemoteProxyBean.#variables.proxyId#.Setup" type="readonly" timeout="5">
			<cfif not StructKeyExists(variables, "constructed") or not variables.constructed>
				...
				<cftry>		
					...
					<cfset remoteFactory = bf.getBean("&MeldForumsRemoteAOP") />
					<cfset variables.target = bf.getBean("MeldForumsRemoteAOP") />
	...
	<cffunction name="getConferenceList" access="remote" returntype="any" output="false" > 
<cfset var rtn = callMethod('getConferenceList', arguments) />
<cfif isDefined('rtn')><cfreturn rtn /></cfif>
</cffunction>

<cffunction name="setMuraManager" access="remote" returntype="any" output="false" > 
<cfargument name="MuraManager" type="any" required="true" /> 
<cfset var rtn = callMethod('setMuraManager', arguments) />
</cffunction>

<cffunction name="getMuraManager" access="remote" returntype="any" output="false" > 
<cfset var rtn = callMethod('getMuraManager', arguments) />
<cfif isDefined('rtn')><cfreturn rtn /></cfif>
</cffunction>

<cffunction name="getMeldForumsManager" access="remote" returntype="any" output="false" > 
<cfset var rtn = callMethod('getMeldForumsManager', arguments) />
<cfif isDefined('rtn')><cfreturn rtn /></cfif>
</cffunction>

<cffunction name="setMeldForumsManager" access="remote" returntype="any" output="false" > 
<cfargument name="MeldForumsManager" type="any" required="true" /> 
<cfset var rtn = callMethod('setMeldForumsManager', arguments) />
</cffunction>

<cffunction name="getForumList" access="remote" returntype="any" output="false" > 
<cfset var rtn = callMethod('getForumList', arguments) />
<cfif isDefined('rtn')><cfreturn rtn /></cfif>
</cffunction>

<cffunction name="getConfigurationList" access="remote" returntype="any" output="false" > 
<cfset var rtn = callMethod('getConfigurationList', arguments) />
<cfif isDefined('rtn')><cfreturn rtn /></cfif>
</cffunction>

<cffunction name="setCriteria" access="remote" returntype="any" output="false" > 
<cfargument name="sData" type="struct" required="true" /> 
<cfargument name="cols" type="struct" required="true" /> 
<cfargument name="defaultSortCol" type="string" required="false" default="1" /> 
<cfset var rtn = callMethod('setCriteria', arguments) />
<cfif isDefined('rtn')><cfreturn rtn /></cfif>
</cffunction>
