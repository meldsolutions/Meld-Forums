...
<!---
	...
	<cffunction name="onMeldForumsProfileFooterRender" output="true" returntype="string">
		<cfargument name="$">		

		<cfset var app 						= $.getPlugin("MeldForums").getApplication() />
		...
		<cfset var data		 				= $.event().getValue('data') />
		<cfset var mfUserBean 				= "" />
		<cfset var userBean 				= "" />
		
		
		<cftry>
		<cfset mfUserBean 				= data.rc.userBean />			<!--- The Forums User Bean --->
		<cfset userBean 				= mfUserBean.getExternalUserBean() />		<!--- The Mura User Bean --->
		<cfcatch></cfcatch>
		</cftry>
